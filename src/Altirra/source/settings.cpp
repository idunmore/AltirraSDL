...
void ATSettingsExchangeHardware(bool write, VDRegistryKey& key) {
	...
	if (write) {
		...
		key.setString("GTIA: Defect mode", ATEnumToString(gtia.GetDefectMode()));
		...
		VDStringA s;
		key.getString("GTIA: Defect mode", s);
		gtia.SetDefectMode(ATParseEnum<ATGTIADefectMode>(s).mValue);
