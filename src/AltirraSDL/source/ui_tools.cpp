...
static struct DiskExplorerState {
	...
	void FormatView() {
		...
		case kDEView_TextWrap:
			// Convert ATASCII to ASCII (simple mapping)
			viewText.reserve(viewData.size());
			for (uint8 c : viewData) {
				if (c == 0x9B) // ATASCII EOL
					viewText += '\n';
...
static void DiskExplorerImportCallback(void *, const char * const *filelist, int) {
	if (!filelist || !filelist[0] || !g_diskExplorer.pFS) return;
	...
	try {
		...
		// Truncate to 8.3 if needed
		if (filename.size() > 12) {
			filename = filename.subspan(0, 8);
			filename += ".???";
		}

		g_diskExplorer.pFS->WriteFile(g_diskExplorer.currentDir,
			filename.c_str(), buf.data(), (uint32)buf.size());
...
static void DiskExplorerExportCallback(void *userdata, const char * const *filelist, int) {
	if (!filelist || !filelist[0] || !g_diskExplorer.pFS) return;

	int idx = (int)(intptr_t)userdata;
	if (idx < 0 || idx >= (int)g_diskExplorer.entries.size()) return;

	try {
		auto &entry = g_diskExplorer.entries[idx];
		vdfastvector<uint8> buf;
		g_diskExplorer.pFS->ReadFile(entry.key, buf);
...
void ATUIRenderDiskExplorer(ATSimulator &sim, ATUIState &state, SDL_Window *window) {
	...
	if (ImGui::SmallButton("Export...") && hasSelection) {
		...
		static const SDL_DialogFileFilter kFilters[] = { { "All Files", "*" } };
		SDL_ShowSaveFileDialog(DiskExplorerExportCallback,
			(void *)(intptr_t)g_diskExplorer.selectedEntry,
			window, kFilters, 1, nullptr);
	}
	ImGui::EndDisabled();
		...
		for (int i = 0; i < (int)g_diskExplorer.entries.size(); ++i) {
			...
			if (ImGui::Selectable(label.c_str(), selected,
				...
				if (ImGui::IsMouseDoubleClicked(ImGuiMouseButton_Left) && e.isDir) {
					if (e.name == "..") {
						ATDiskFSKey parent = g_diskExplorer.pFS->GetParentDirectory(g_diskExplorer.currentDir);
						g_diskExplorer.NavigateTo(parent);
					} else {
						g_diskExplorer.NavigateTo(e.key);
					}
				}
			}
			...
			if (ImGui::BeginPopupContextItem()) {
				if (!e.isDir && ImGui::MenuItem("Export...")) {
					static const SDL_DialogFileFilter kFilters[] = { { "All Files", "*" } };
					SDL_ShowSaveFileDialog(DiskExplorerExportCallback,
						(void *)(intptr_t)i, window, kFilters, 1, nullptr);
				}
	...
	static const char *kViewModeNames[] = { "Text", "Text (wrap)", "Hex Dump", "Executable" };
...
static void CompatDBLoadCallback(void *, const char * const *filelist, int) {
	...
	try {
		...
		// Override known tag display names
...
void ATUIRenderCompatDB(ATSimulator &sim, ATUIState &state) {
	...
	if (!ImGui::Begin(caption.c_str(), &open, ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_MenuBar)) {
		if (!open) { g_compatDB.Reset(); state.showCompatDB = false; }
		ImGui::End();
		return;
	}
