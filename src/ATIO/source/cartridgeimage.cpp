...
namespace {
	...
	uint32 ComputeByteSum32(const uint8 *src, size_t len) {
		...
		while(len64) {
			uint16x8_t acc0 = vmovq_n_u16(0);
			uint16x8_t acc1 = vmovq_n_u16(0);
			uint16x8_t acc2 = vmovq_n_u16(0);
			uint16x8_t acc3 = vmovq_n_u16(0);

			size_t n64 = std::min<size_t>(len64, 127);
			len64 -= n64;

			while(n64--) {
				uint8x16x4_t v = vld1q_u8_x4(src);

				acc0 = vpadalq_u8(acc0, v.val[0]);
				acc1 = vpadalq_u8(acc1, v.val[1]);
				acc2 = vpadalq_u8(acc2, v.val[2]);
				acc3 = vpadalq_u8(acc3, v.val[3]);

				src += 64;
			}

			sum += vaddvq_u32(vpadalq_u16(vpadalq_u16(vpadalq_u16(vpaddlq_u16(acc0), acc1), acc2), acc3));
		}
...
bool ATCartridgeImage::Load(const wchar_t *path, IVDRandomAccessStream& stream, ATCartLoadContext *loadCtx) {
	...
	if (!memcmp(buf, "CART", 4)) {
		...
		bool useChecksum = (!loadCtx || !loadCtx->mbIgnoreChecksum) && checksum != 0;
		uint32 csum = 0;
