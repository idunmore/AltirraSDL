...
ATOM ATUINativeWindow::Register() {
	...
	WNDCLASS wc = {};
	wc.lpszClassName	= L"ATUINativeWindow";
...
ATUINativeWindow *ATUINativeWindow::BaseFromHandle(VDZHWND hwnd) {
	return hwnd ? (ATUINativeWindow *)GetWindowLongPtr(hwnd, 0) : nullptr;
}
...
bool ATUINativeWindow::Create(int x, int y, int w, int h, bool visible, const wchar_t *text) {
	DWORD dwFlags = WS_POPUP;

	if (visible)
		dwFlags |= WS_VISIBLE;

	return CreateWindowExW(0, MAKEINTATOM(sWndClass), text ? text : L"", dwFlags, x, y, w, h, nullptr, nullptr, VDGetLocalModuleHandleW32(), this) != nullptr;
}

bool ATUINativeWindow::CreateChild(ATUINativeWindowProxy& parent, uint32 id, int x, int y, int w, int h, bool visible, const wchar_t *text) {
	return CreateChild(parent.GetWindowHandle(), id, x, y, w, h, WS_CHILD | (visible ? WS_VISIBLE : 0), 0, text);
}
