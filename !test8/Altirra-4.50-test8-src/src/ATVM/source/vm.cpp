...
const ATVMDebugSourceReference *ATVMFunction::TryGetDebugSourceRefFromCodePos(const uint8 *ip) const {
	if (!ip || mDebugSourceRanges.empty())
		return nullptr;

	const uint32 ipOffset = (uint32)(ip - mpByteCode);

	// find the last entry with offset <= ipOffset
	ATVMDebugSourceRange testRange {};
	testRange.mCodeOffset = ipOffset;

	auto it = std::ranges::upper_bound(mDebugSourceRanges, testRange,
		[](const ATVMDebugSourceRange& x, const ATVMDebugSourceRange& y) {
			return x.mCodeOffset < y.mCodeOffset;
		}
	);

	if (it == mDebugSourceRanges.begin())
		return nullptr;

	--it;
	return &it->mSourceRef;
}
...
const char *ATVMDomain::StoreString(const char *s) {
	size_t len = strlen(s);

	char *t = mAllocator.AllocateArray<char>(len + 1);
	memcpy(t, s, len + 1);
