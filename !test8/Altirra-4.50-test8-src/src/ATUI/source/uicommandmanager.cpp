...
void ATUICommandContext::MarkCompleted(bool succeeded) {
	if (!mbCommandCompleted) {
		...
		if (mpOnCommandExecuted) {
			if (mbCommandCancelled)
				mpOnCommandExecuted(nullptr, {});
			else
				mpOnCommandExecuted(mpCommand, mOutArgs);
		}
	}
}
...
void ATUICommandManager::RegisterCommand(const ATUICommand *cmd) {
	const uint32 hash = cmd->mName.mNameHash;
...
const ATUICommand *ATUICommandManager::GetCommand(const char *str) const {
	ATUICommandName name;
	name.mpName = str;
	name.mNameHash = VDHashString32(str);

	return GetCommand(name);
}

const ATUICommand *ATUICommandManager::GetCommand(const ATUICommandName& name) const {
	const uint32 hash = name.mNameHash;
	const uint32 htidx = hash % kHashTableSize;

	for(const Node *node = mpHashTable[htidx]; node; node = node->mpNext) {
		const ATUICommand *cmd = node->mpCmd;

		if (cmd->mName.mNameHash == hash && !strcmp(cmd->mName.mpName, name.mpName)) {
			return cmd;
		}
	}
...
bool ATUICommandManager::IsCommandAvailable(const ATUICommandName& name) const {
	const ATUICommand *cmd = GetCommand(name);
...
bool ATUICommandManager::ExecuteCommand(const ATUICommandName& name, const ATUICommandOptions& ctx) {
	const ATUICommand *cmd = GetCommand(name);
...
bool ATUICommandManager::ExecuteCommand(const ATUICommand& cmd, const ATUICommandOptions& ctx) {
	...
	static_cast<ATUICommandBaseOptions&>(*mpCommandContext) = ctx;

	mpCommandContext->mInArgs.assign(ctx.mInArgs.begin(), ctx.mInArgs.end());
	...
	mpCommandContext->mbRecordArgs = false;
	mpCommandContext->mOutArgs.clear();

	const auto initialRC = mpCommandContext->GetRefCount();
	...
	if (mpCommandContext->GetRefCount() == initialRC)
...
bool ATUICommandManager::ExecuteCommandNT(const ATUICommandName& cmd, const ATUICommandOptions& ctx) noexcept {
	try {
		ExecuteCommand(cmd, ctx);
	} catch(const MyError&) {
		return false;
...
void ATUICommandManager::ListCommands(vdfastvector<VDAccelToCommandEntry>& commands) const {
	for(uint32 i=0; i<kHashTableSize; ++i) {
		for(const Node *node = mpHashTable[i]; node; node = node->mpNext) {
			...
			ace.mId = 0;
			ace.mpName = node->mpCmd->mName.mpName;
		}
	}
}
