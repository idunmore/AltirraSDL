...
public:
	...
	void SetOnFormatDetectFormat(vdfunction<void(uint32 halfTrack, bool side2)> fn) {
		mpFnFormatDetect = std::move(fn);
	}
	...
	enum class SideMapping : uint32 {
		...
		// Side 2 tracks are interleaved with side 1 tracks. [Karin Maxi Drive]
		Side2Interleaved
	};
...
protected:
	...
	vdfunction<void(uint32 halfTrack, bool side2)> mpFnFormatDetect;
