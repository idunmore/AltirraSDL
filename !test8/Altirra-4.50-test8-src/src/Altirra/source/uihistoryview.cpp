...
public:
	...
	void SetTimestampMode(ATUIHistoryTimestampMode mode) override;
	void SetTimestampOrigin(uint32 cycles, uint32 unhaltedCycles) override;
	void SetTimestampBase(uint32 cycles, uint32 unhaltedCycles) override;
...
protected:
	...
	void InternalSetTimestampMode(ATUIHistoryTimestampMode mode, bool notify);
	void InternalSetTimestampOrigin(uint32 cycles, uint32 unhaltedCycles, bool notify);
	...
	using TimestampMode = ATUIHistoryTimestampMode;
	static constexpr auto
		kTsMode_Beam = ATUIHistoryTimestampMode::Beam,
		kTsMode_Microseconds = ATUIHistoryTimestampMode::Microseconds,
		kTsMode_Cycles = ATUIHistoryTimestampMode::Cycles,
		kTsMode_UnhaltedCycles = ATUIHistoryTimestampMode::UnhaltedCycles,
		kTsMode_TapePositionSamples = ATUIHistoryTimestampMode::TapePositionSamples,
		kTsMode_TapePositionSeconds = ATUIHistoryTimestampMode::TapePositionSeconds;
	...
	vdfunction<void(uint32, uint32)> mpOnTimestampOriginChanged;
...
void ATUIHistoryView::SetTimestampMode(ATUIHistoryTimestampMode mode) {
	InternalSetTimestampMode(mode, false);
}

void ATUIHistoryView::SetTimestampOrigin(uint32 cycles, uint32 unhaltedCycles) {
	InternalSetTimestampOrigin(cycles, unhaltedCycles, false);
}

void ATUIHistoryView::SetTimestampBase(uint32 cycles, uint32 unhaltedCycles) {
...
void ATUIHistoryView::SetOnTimestampModeChanged(vdfunction<void(ATUIHistoryTimestampMode)> fn) {
	mpOnTimestampModeChanged = std::move(fn);
}

void ATUIHistoryView::SetOnTimestampOriginChanged(vdfunction<void(uint32, uint32)> fn) {
		...
		case WM_COMMAND:
			switch(LOWORD(wParam)) {
				...
				case ID_HISTORYCONTEXTMENU_SHOWBEAMPOSITION:
					InternalSetTimestampMode(kTsMode_Beam, true);
					return true;

				case ID_HISTORYCONTEXTMENU_SHOWMICROSECONDS:
					InternalSetTimestampMode(kTsMode_Microseconds, true);
					return true;

				case ID_HISTORYCONTEXTMENU_SHOWCYCLES:
					InternalSetTimestampMode(kTsMode_Cycles, true);
					return true;

				case ID_HISTORYCONTEXTMENU_SHOWUNHALTEDCYCLES:
					InternalSetTimestampMode(kTsMode_UnhaltedCycles, true);
					return true;

				case ID_HISTORYCONTEXTMENU_SHOWTAPEPOSITIONSAMPLES:
					InternalSetTimestampMode(kTsMode_TapePositionSamples, true);
					return true;

				case ID_HISTORYCONTEXTMENU_SHOWTAPEPOSITIONSECONDS:
					InternalSetTimestampMode(kTsMode_TapePositionSeconds, true);
					return true;

				case ID_HISTORYCONTEXTMENU_RESETTIMESTAMPORIGIN:
					InternalSetTimestampOrigin(mTimeBaseCyclesDefault, mTimeBaseUnhaltedCyclesDefault, true);
					return true;

				case ID_HISTORYCONTEXTMENU_SETTIMESTAMPORIGIN:
					if (const ATCPUHistoryEntry *he = GetSelectedLineHistoryEntry()) {
						InternalSetTimestampOrigin(he->mCycle, he->mUnhaltedCycle, true);
					}
...
void ATUIHistoryView::InternalSetTimestampMode(ATUIHistoryTimestampMode mode, bool notify) {
	if (mTimestampMode != mode) {
		mTimestampMode = mode;
...
void ATUIHistoryView::InternalSetTimestampOrigin(uint32 cycles, uint32 unhaltedCycles, bool notify) {
	if (mTimeBaseCycles != cycles || mTimeBaseUnhaltedCycles != unhaltedCycles) {
		mTimeBaseCycles = cycles;
		mTimeBaseUnhaltedCycles = unhaltedCycles;

		if (notify && mpOnTimestampOriginChanged)
			mpOnTimestampOriginChanged(cycles, unhaltedCycles);
