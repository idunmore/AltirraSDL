...
void ATTraceChannelTickBased::StartIterationNearest(double startTime) {
	// Find first event at or after requested start time.
	auto it = mEvents.cbegin();
	...
	if (!mEvents.empty()) {
		it = std::lower_bound(
			...
			mEvents.cend(),
			startTime,
			[](const SimpleEvent& ev, double t) {
				return ev.mStartTime < t;
			}
		);

		// Check if we can back up.
		if (it != mEvents.cbegin()) {
			// If we're at the end, we should unconditionally back up.
			if (it == mEvents.cend()) {
				--it;
			} else {
				// If we're not already in the next event, back up if the
				// distance to the previous event's end time is <= the current
				// event's start time.
				const double timeToNext = startTime - it->mStartTime;

				if (timeToNext > 0 && timeToNext > std::prev(it)->mEndTime)
					--it;
			}
		}
	}
...
uint32 ATTraceChannelStringTable::AddString(const wchar_t *s) {
	const uint32 id = (uint32)mStringTable.size();
...
uint32 ATTraceChannelStringTable::AddString(VDStringW&& s) {
	const uint32 id = (uint32)mStringTable.size();

	mStringTable.emplace_back(std::move(s));
