...
void ATUIPrinterGraphicalOutputWindow::SaveAsSVG() {
	...
	const ATPrinterGraphicsSpec& spec = mpOutput->GetGraphicsSpec();
	vdrect32f docBounds = mpOutput->GetDocumentBounds();
	...
	if (mpOutput->PreCull(cullInfo, docBounds)) {
		vdfastvector<ATPrinterGraphicalOutput::RenderColumn> cols;
		float rawLineY = 0;
		while(mpOutput->ExtractNextLine(cols, rawLineY, cullInfo, docBounds)) {
			uint32 allDotMask = 0;
	...
	vdfastvector<ATPrinterGraphicalOutput::RenderVector> rvectors;
	mpOutput->ExtractVectors(rvectors, docBounds);
