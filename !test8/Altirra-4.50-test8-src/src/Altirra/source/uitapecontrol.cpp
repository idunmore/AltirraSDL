...
extern const ATUICommandName g_ATUICmdNameCassetteLoadNew;
extern const ATUICommandName g_ATUICmdNameCassetteLoad;
extern const ATUICommandName g_ATUICmdNameCassetteSave;
extern const ATUICommandName g_ATUICmdNameCassetteSaveAs;
extern const ATUICommandName g_ATUICmdNameCassetteUnload;
...
class ATTapeControlDialog final : public VDDialogFrameW32 {
public:
...
void ATTapeControlDialog::New() {
	ATUIGetCommandManager().ExecuteCommand(g_ATUICmdNameCassetteLoadNew);
}

void ATTapeControlDialog::Load() {
	ATUIGetCommandManager().ExecuteCommand(g_ATUICmdNameCassetteLoad);
}

void ATTapeControlDialog::Save() {
	ATUIGetCommandManager().ExecuteCommand(g_ATUICmdNameCassetteSave);
}

void ATTapeControlDialog::SaveAs() {
	ATUIGetCommandManager().ExecuteCommand(g_ATUICmdNameCassetteSaveAs);
}

void ATTapeControlDialog::Unload() {
	ATUIGetCommandManager().ExecuteCommand(g_ATUICmdNameCassetteUnload);
}
...
bool ATTapeControlDialog::OnLoaded() {
	...
	mResizer.Add(IDC_STOP, mResizer.kBL | mResizer.kSuppressFontChange);
	mResizer.Add(IDC_PAUSE, mResizer.kBL | mResizer.kSuppressFontChange);
	mResizer.Add(IDC_PLAY, mResizer.kBL | mResizer.kSuppressFontChange);
	mResizer.Add(IDC_RECORD, mResizer.kBL | mResizer.kSuppressFontChange);
	mResizer.Add(IDC_SEEK_START, mResizer.kBL | mResizer.kSuppressFontChange);
	mResizer.Add(IDC_SEEK_END, mResizer.kBL | mResizer.kSuppressFontChange);
	mResizer.Add(IDC_POSITION, mResizer.kBC);
	mResizer.Add(IDC_PEAK_IMAGE, mResizer.kMC);
	mResizer.Add(IDC_STATIC_POSITION, mResizer.kBR);
...
void ATTapeControlDialog::OnSize() {
...
void ATTapeControlDialog::OnInitMenu(VDZHMENU hmenu) {
	auto& cm = ATUIGetCommandManager();

	VDEnableMenuItemByCommandW32(hmenu, ID_FILE_SAVE, cm.IsCommandAvailable(g_ATUICmdNameCassetteSave));
	VDEnableMenuItemByCommandW32(hmenu, ID_FILE_SAVEAS, cm.IsCommandAvailable(g_ATUICmdNameCassetteSaveAs));
	VDEnableMenuItemByCommandW32(hmenu, ID_FILE_UNLOAD, cm.IsCommandAvailable(g_ATUICmdNameCassetteUnload));
}

bool ATTapeControlDialog::OnCommand(uint32 id, uint32 code) {
	switch(id) {
		case ID_FILE_NEW:
			New();
			return true;
