...
namespace {
	...
	vdfunction<void(const char *, uint64)> g_ATConsoleTraceLogger;
}
...
bool ATRestorePaneLayout(const char *name) {
	...
	if (s) {
		while(*s++ == ';') {
			...
			// If we already have this frame, activate it; otherwise, create it.
			vdhashmap<uint32, ATFrameWindow *>::iterator it(frames.find(id));
			ATFrameWindow *w = NULL;
			...
			x1 = std::clamp<sint32>(x1, -32768, 32767);
			y1 = std::clamp<sint32>(y1, -32768, 32767);
			x2 = std::clamp<sint32>(x2, -32768, 32767);
			y2 = std::clamp<sint32>(y2, -32768, 32767);
			...
			// Determine nearest monitor for the proposed window area, and use that to obtain DPI.
			const RECT rWin { x1, y1, x2, y2 };
			const HMONITOR hmonNearest = ::MonitorFromRect(&rWin, MONITOR_DEFAULTTONEAREST);
			const uint32 nearestMonitorDpi = ATUIGetMonitorDpiW32(hmonNearest);

			// Compute reasonable minimum size for the window
			const int minWidth = ATUIGetDpiScaledSystemMetricW32(SM_CXMIN, nearestMonitorDpi);
			const int minHeight = ATUIGetDpiScaledSystemMetricW32(SM_CYMIN, nearestMonitorDpi);

			x2 = std::max<sint32>(x2, x1 + minWidth);
			y2 = std::max<sint32>(y2, y1 + minHeight);
			...
			if (!hmon && hmonNearest) {
				...
				if (GetMonitorInfo(hmonNearest, &mi) && GetTitleBarInfo(hwndFrame, &tbi) && GetWindowRect(hwndFrame, &r)) {
					// Move the window so that the entire title bar is within the work area of the monitor,
					// or as much as we can manage.
					const sint32 tbWidth = tbi.rcTitleBar.right - tbi.rcTitleBar.left;
					const sint32 tbHeight = tbi.rcTitleBar.bottom - tbi.rcTitleBar.top;
					const sint32 wsWidth = mi.rcWork.right - mi.rcWork.left;
					const sint32 wsHeight = mi.rcWork.bottom - mi.rcWork.top;

					sint32 newX = r.left;
					sint32 newY = r.top;

					if (tbWidth >= wsWidth || newX < mi.rcWork.left)
						newX = mi.rcWork.left;
					else if (r.right > mi.rcWork.right)
						newX += mi.rcWork.right - r.right;

					if (tbHeight >= wsHeight || newY < mi.rcWork.top)
						newY = mi.rcWork.top;
					else if (r.bottom > mi.rcWork.bottom)
						newY += mi.rcWork.bottom - r.bottom;

					SetWindowPos(hwndFrame, nullptr, newX, newY, 0, 0, SWP_NOSIZE | SWP_NOZORDER | SWP_NOACTIVATE);
				}
			}
...
void ATConsoleSetTraceLogger(vdfunction<void(const char *, uint64)> traceLogger) {
	g_ATConsoleTraceLogger = traceLogger;
}

void ATConsoleWrite(const char *s) {
	if (g_ATConsoleTraceLogger)
		g_ATConsoleTraceLogger(s, g_sim.GetScheduler()->GetTick64());
