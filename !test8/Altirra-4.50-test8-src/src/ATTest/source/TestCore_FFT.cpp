...
AT_DEFINE_TEST(Core_FFT) {
	...
	for(int pass = 0; pass < 2; ++pass) {
		const bool speed = pass > 0;

		vdautoptr<ATFFT<16>> fft16(new ATFFT<16>(speed));
		vdautoptr<ATFFT<32>> fft32(new ATFFT<32>(speed));
		vdautoptr<ATFFT<64>> fft64(new ATFFT<64>(speed));
		vdautoptr<ATFFT<2048>> fft2k(new ATFFT<2048>(speed));
		vdautoptr<ATFFT<4096>> fft4k(new ATFFT<4096>(speed));
		...
		// we only test a subset of the 4K positions for speed reasons
		double fwdRelError = 0;
		double invRelError = 0;
		...
		for(int i=0; i<4096; i += 37) {
			...
			// spot check only a single reference bucket, since it's very slow
			if (i == 37*13) {
				const double div = -nsVDMath::krTwoPi / 4096.0;
				for(int j=0; j<4096; ++j) {
					dat->costab[j] = cos(div * j);
				}
			...
			if (i == 37*13) {
				...
				for(int j=0; j<4096; ++j) {
					double r = (double)dat->ref[j];
					double e = (double)dat->x[j] - r;
				...
				fwdRelError = sqrt(errPower / refPower);
			}
			...
			if (i == 37*13) {
				...
				double refPower = 2048.0 * 2048.0;

				invRelError = sqrt(errPower / refPower);
			}
		...
		AT_TEST_TRACEF("FFT4K pass - %g %g", fwdRelError, invRelError);
	}
...
AT_DEFINE_TEST_NONAUTO(Core_FFTBench) {
	auto testSize = [](auto N, bool speed) {
		const char *tag = speed ? " (optimized for speed)" : " (optimized for efficiency)";
		ATFFT<N> fft(speed);
...

		{
			...
			printf("%7.3fus  %9.1f MFLOPS  %4d-point forward FFT%s\n", usecs, mflops, (int)N, tag);
		}

		{
			...
			printf("%7.3fus  %9.1f MFLOPS  %4d-point inverse FFT%s\n", usecs, mflops, (int)N, tag);
		}
	};
	...
	for(int pass = 0; pass < 2; ++pass) {
		...
		testSize(std::integral_constant<int, 64>(), speed);
		testSize(std::integral_constant<int, 128>(), speed);
		testSize(std::integral_constant<int, 256>(), speed);
		testSize(std::integral_constant<int, 512>(), speed);
		testSize(std::integral_constant<int, 1024>(), speed);
		testSize(std::integral_constant<int, 2048>(), speed);
		testSize(std::integral_constant<int, 4096>(), speed);
		testSize(std::integral_constant<int, 8192>(), speed);
	}
