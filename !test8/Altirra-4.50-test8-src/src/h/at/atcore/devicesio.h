...
public:
	...
	// Set the number of machine cycles between each byte after which reception of a
	// command or data frame will time out and cancel the command on the device. The
	// default is infinite / no timeout.
	virtual void SetReceiveByteTimeout(uint64 cycles = UINT64_MAX) = 0;

	// Set the number of machine cycles after which reception of a data
	// frame will time out and cancel the command on the device. The default is
	// infinite / no timeout.
	//
	// If both byte and frame timeouts are set, the earlier timeout to expire matters.
	virtual void SetReceiveFrameTimeout(uint64 cycles = UINT64_MAX) = 0;
