...
class ATUICommandName {
public:
	ATUICommandName() = default;

	consteval ATUICommandName(const char *s)
		: mpName(s)
		, mNameHash(VDHashString32C(s))
	{
	}
...
public:
	...
	const ATUICommand *GetCommand(const ATUICommandName& name) const;
	bool IsCommandAvailable(const ATUICommandName& name) const;

	bool ExecuteCommand(const char *str, const ATUICommandOptions& ctx = ATUICommandOptions());
	bool ExecuteCommand(const ATUICommandName& cmd, const ATUICommandOptions& ctx = ATUICommandOptions());
	...
	bool ExecuteCommandNT(const ATUICommandName& cmd, const ATUICommandOptions& ctx = ATUICommandOptions()) noexcept;
