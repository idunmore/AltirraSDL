...
struct VDTDisplayColorInfo {
	// CIE 1931 xy coordinates of RGB primaries and white point. These will all
	// be zero if not available.
...
public:
	...
	// Attempt to retrieve the color space of the display; returns true if success.
	virtual bool GetDisplayColorInfo(VDTDisplayColorInfo& info) const = 0;
...
public:
	...
	virtual bool IsMonitorHighColorEnabled(void *monitor, bool& systemSupport) = 0;
