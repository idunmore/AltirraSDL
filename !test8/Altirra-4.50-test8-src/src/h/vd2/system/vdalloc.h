...
public:
	...
	vdnothrow vdautoptr& operator=(vdautoptr<U>&& src) noexcept {
		delete ptr;
		ptr = src.release();
...
template<typename T>
concept VDClassType = std::is_class_v<T>;

template<class T>
class vddeleterholder {
	T deleter;
};

template<VDClassType T>
class vddeleterholder<T> : public T {};
...
public:
	vduniqueptr(const vduniqueptr&) = delete;
	vduniqueptr& operator=(vduniqueptr&) = delete;

	vdnothrow vduniqueptr(std::nullptr_t) noexcept {}
	vdnothrow explicit vduniqueptr(T *p = nullptr) noexcept
		: ptr(p)
	{
	}

	vdnothrow vduniqueptr(vduniqueptr&& src) noexcept
		: ptr(src.ptr)
	{
		src.ptr = nullptr;
	}
	...
	template<class U, typename = std::enable_if<std::is_convertible_v<U, T>>>
	vdnothrow vduniqueptr(vduniqueptr<U>&& src) noexcept
		: ptr(src.release())
	{
	}
	...
	vdnothrow vduniqueptr& operator=(vduniqueptr&& src) noexcept {
		get_deleter()(ptr);
		ptr = nullptr;
...
vduniqueptr<T> vdmakeunique(Args&&... args) {
	return vduniqueptr<T>(new T(std::forward<Args>(args)...));
}
