...
bool VDTSwapChainD3D11::GetDisplayColorInfo(VDTDisplayColorInfo& info) const {
	if (!mpSwapChain)
		return false;

	vdrefptr<IDXGIOutput> output;
	mpSwapChain->GetContainingOutput(~output);
	...
	info.mRedPrimary.x = desc1.RedPrimary[0];
	info.mRedPrimary.y = desc1.RedPrimary[1];
	info.mGreenPrimary.x = desc1.GreenPrimary[0];
	info.mGreenPrimary.y = desc1.GreenPrimary[1];
	info.mBluePrimary.x = desc1.BluePrimary[0];
	info.mBluePrimary.y = desc1.BluePrimary[1];
	info.mWhitePoint.x = desc1.WhitePoint[0];
	info.mWhitePoint.y = desc1.WhitePoint[1];
	return true;
}
...
void VDTSwapChainD3D11::ThreadRun() {
	VDDXGIHolder *pHolder = static_cast<VDTContextD3D11 *>(mpParent)->GetDXGIHolder();
...
bool VDTContextD3D11::Init(ID3D11Device *dev, ID3D11DeviceContext *devctx, IDXGIAdapter1 *adapter, IDXGIFactory *factory, VDDXGIHolder *pDXGIHolder, VDD3D11Holder *pD3DHolder) {
...
void VDTContextD3D11::Shutdown() {
	...
	vdsaferelease <<= mpDXGIAdapter, mpDXGIFactory;
	...
	vdsaferelease <<= mpD3DHolder, mpDXGIHolder;
}
...
bool VDTContextD3D11::IsMonitorHighColorEnabled(void *monitor, bool& systemSupport) {
	vdrefptr<IDXGIOutput> output;
	systemSupport = false;
...
bool VDTCreateContextD3D11(IVDTContext **ppctx) {
	...
	vdrefptr<VDDXGIHolder> dxgiHolder(new VDDXGIHolder);

	if (!holder->Init() || !dxgiHolder->Init())
		return false;
	...
	HRESULT hr = E_FAIL;
	if (dxgiHolder->GetCreateDXGIFactory2Fn()) {
#ifdef _DEBUG
		...
		hr = dxgiHolder->GetCreateDXGIFactory2Fn()(flags, IID_IDXGIFactory3, (void **)~factory3);
	...
	if (!factory)
		hr = dxgiHolder->GetCreateDXGIFactoryFn()(IID_IDXGIFactory1, (void **)~factory);
	...
	if (!ctx->Init(dev, devctx, adapter, factory, dxgiHolder, holder))
		return false;
...
bool VDTCreateContextD3D11(ID3D11Device *dev, ID3D11DeviceContext *devctx, IDXGIFactory *factory, IVDTContext **ppctx) {
	...
	if (!ctx->Init(dev, devctx, adapter1, factory, nullptr, nullptr))
		return false;
