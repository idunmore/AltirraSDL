	...
	if (cmd == 0x52) {
		...
		if (mpDiskInterface->IsDiskLoaded()) {
			...
			if (sector <= img.GetVirtualSectorCount()) {
				...
				// find closest sector
				for(const auto& psec : img.GetPhysicalSectorsForVirtualSector(sector - 1)) {
					// if this sector has an address CRC, it will be shadowed by a sector
					// that doesn't
					if (!(psec.mPhysInfo.mFDCStatus & 0x18) && (bestStatus & 0x18))
						continue;

					// compute angular distance
					float dist = psec.mPhysInfo.mRotPos - rot;
					dist -= floorf(dist);

					if (dist < bestDistance) {
						bestIndex = (int)psec.mPhysOrder;
						bestDistance = dist;
						bestStatus = psec.mPhysInfo.mFDCStatus;
						bestPhysSector = psec.mPhysIndex;
						bestWeakOffset = psec.mPhysInfo.mWeakDataOffset;
					}
				}
			}
