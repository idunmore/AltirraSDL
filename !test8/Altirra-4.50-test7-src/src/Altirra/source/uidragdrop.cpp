...
ATLogChannel g_ATLCHostDragDrop(false, false, "HOSTDRAGDROP", "Host drag-and-drop UI operations");
...
public:
	...
	uint32 GetFilePathCount() const override;
	bool GetFilePath(VDStringW& path) const override;
...
uint32 ATUIWrappedDataObject::GetFilePathCount() const {
	...
	HRESULT hr = mpObject->GetData(&etc, &medium);
	...
	return DragQueryFileW(hdrop, 0xFFFFFFFFU, nullptr, 0);
}

bool ATUIWrappedDataObject::GetFilePath(VDStringW& path) const {
	ATAutoStgMediumW32 medium;
	medium.tymed = TYMED_HGLOBAL;
	medium.hGlobal = NULL;
	...
	vdfastvector<WCHAR> buf(len + 1);
	UINT copiedLen = DragQueryFileW(hdrop, 0, buf.data(), buf.size());
	if (!copiedLen)
		return false;

	path.assign(buf.data(), buf.data() + copiedLen);
	return true;
}
...
bool ATUICanDropFiles(IATUIDragDropObject *obj) {
	...
	return ATUICanDropFiles(static_cast<IDataObject *>(obj->GetNativeObject()));
}
...
void ATUIDropFiles2(POINTL pt, ATUIDropFilesTarget target, IDataObject *pDataObj);

void ATUIDropFiles(POINTL pt, ATUIDropFilesTarget target, IDataObject *pDataObj) {
	try {
		ATUIDropFiles2(pt, target, pDataObj);
	} catch(MyError e) {
		ATUIPushStep(
			[e2 = VDException(std::move(e))] {
				throw e2;
			}
		);
	}
...
void ATUIDropFiles2(POINTL pt, ATUIDropFilesTarget target, IDataObject *pDataObj) {
	...
	switch(target) {
		...
		case ATUIDropFilesTarget::PasteFilePath:
			{
				vdrefptr wrappedObj { new ATUIWrappedDataObject(pDataObj) };

				if (VDStringW path; wrappedObj->GetFilePath(path)) {
					VDStringA cioPath;
					for(IATDeviceNativePathMapping *iface : g_sim.GetDeviceManager()->GetInterfaces<IATDeviceNativePathMapping>(false, false, false)) {
						if (iface->MapNativePathToCIOPath(cioPath, path.c_str())) {
							ATUIPaste(cioPath);
							return;
						}
					}
	...
	g_ATLCHostDragDrop("GetData(SHIDL) hr=%08X\n", hr);
	if (hr == S_OK) {
		VDStringW vfsPath;
		...
		if (ATUIGetVFSPathFromShellIDListW32(medium.hGlobal, vfsPath)) {
			...
			try {
				...
				// Eat VFS open errors. We assume that it's something like a .zip we can't handle,
				// and fall through.
			}
	...
	g_ATLCHostDragDrop("GetData(HDROP) hr=%08X\n", hr);
	...
	if (hr == S_OK) {
		...
		hr = pDataObj->GetData(&etc, &medium);
		g_ATLCHostDragDrop("GetData(DESCW) hr=%08X\n", hr);
		...
		if (hr == S_OK) {
			...
			g_ATLCHostDragDrop("GetData(DESCA) hr=%08X\n", hr);
			hr = pDataObj->GetData(&etc, &medium);
...
void ATUIDropFiles(const vdpoint32& pt, ATUIDropFilesTarget target, IATUIDragDropObject *obj) {
	if (!obj) {
		g_ATLCHostDragDrop("Skipping, no data object\n");
	}
	...
	g_ATLCHostDragDrop("Processing drag drop\n");
...
HRESULT STDMETHODCALLTYPE ATUIDragDropHandler::DragEnter(IDataObject *pDataObj, DWORD grfKeyState, POINTL pt, DWORD *pdwEffect) {
	...
	g_ATLCHostDragDrop("DragEnter begin (%p)\n", mpWrappedDataObject.get());

	WrapDataObject(nullptr);
	...
	g_ATLCHostDragDrop("DragEnter end\n");
...
HRESULT STDMETHODCALLTYPE ATUIDragDropHandler::DragOver(DWORD grfKeyState, POINTL pt, DWORD *pdwEffect) {
	g_ATLCHostDragDrop("DragOver begin\n");
	...
	g_ATLCHostDragDrop("DragOver end\n");
...
HRESULT STDMETHODCALLTYPE ATUIDragDropHandler::DragLeave() {
	g_ATLCHostDragDrop("DragLeave begin\n");
...
HRESULT STDMETHODCALLTYPE ATUIDragDropHandler::Drop(IDataObject *pDataObj, DWORD grfKeyState, POINTL pt, DWORD *pdwEffect) {
	...
	if (!(GetWindowLong(mhwnd, GWL_STYLE) & WS_DISABLED)) {
		if (g_ATDropSubTargetWindow && !mbOpenContextMenu) {
			...
			POINT cpt { pt.x, pt.y };
			if (ScreenToClient(g_ATDropSubTargetWindow, &cpt)) {
				g_ATLCHostDragDrop("OnDragDrop(%d, %d)\n", cpt.x, cpt.y);
