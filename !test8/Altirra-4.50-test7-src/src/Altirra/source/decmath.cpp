...
void ATFPAccelContext::SetFlagC() {
	mCpu.SetFlagC();
}

void ATFPAccelContext::ClearFlagC() {
	mCpu.ClearFlagC();
}

uint8 ATFPAccelContext::ReadByte(uint16 addr) const {
	return mMem.ReadByte(addr);
}

void ATFPAccelContext::WriteByte(uint16 addr, uint8 value) {
	mMem.WriteByte(addr, value);
}
...
ATDecFloat ATReadDecFloat(IATFPAccelContext& ctx, uint16 addr) {
	ATDecFloat v;

	v.mSignExp		= ctx.ReadByte(addr);
	v.mMantissa[0]	= ctx.ReadByte(addr+1);
	v.mMantissa[1]	= ctx.ReadByte(addr+2);
	v.mMantissa[2]	= ctx.ReadByte(addr+3);
	v.mMantissa[3]	= ctx.ReadByte(addr+4);
	v.mMantissa[4]	= ctx.ReadByte(addr+5);
	return v;
}
...
void ATWriteDecFloat(IATFPAccelContext& ctx, uint16 addr, const ATDecFloat& v) {
	ctx.WriteByte(addr, v.mSignExp);
	ctx.WriteByte(addr+1, v.mMantissa[0]);
	ctx.WriteByte(addr+2, v.mMantissa[1]);
	ctx.WriteByte(addr+3, v.mMantissa[2]);
	ctx.WriteByte(addr+4, v.mMantissa[3]);
	ctx.WriteByte(addr+5, v.mMantissa[4]);
}

ATDecFloat ATReadFR0(IATFPAccelContext& ctx) {
	return ATReadDecFloat(ctx, ATKernelSymbols::FR0);
}
...
ATDecFloat ATReadFR1(IATFPAccelContext& ctx) {
	return ATReadDecFloat(ctx, ATKernelSymbols::FR1);
}
...
void ATWriteFR0(IATFPAccelContext& ctx, const ATDecFloat& x) {
	return ATWriteDecFloat(ctx, ATKernelSymbols::FR0, x);
}
...
void ATAccelFASC(ATCPUEmulator& cpu, ATCPUEmulatorMemory& mem) {
	ATFPAccelContext ctx(cpu, mem);
	return ATAccelFASC(ctx);
}
...
void ATAccelFASC(IATFPAccelContext& ctx) {
	...
	char buf[21];
	char *s = buf;
	const ATDecFloat v(ATReadFR0(ctx));

	uint16 basePtr = ATKernelSymbols::LBUFF;
	...
	ctx.WriteByte(ATKernelSymbols::INBUFF, (uint8)basePtr);
	ctx.WriteByte(ATKernelSymbols::INBUFF+1, (uint8)(basePtr >> 8));
	...
	for(int i=0; i<len - 1; ++i) {
		...
		ctx.WriteByte(basePtr + i, c);
	}

	// required by Darg
	if (basePtr > ATKernelSymbols::LBUFF)
		ctx.WriteByte(ATKernelSymbols::LBUFF, 0x30);

	ctx.WriteByte(basePtr + len - 1, (uint8)(s[-1] | 0x80));
	*s = 0;

	// SysInfo 2.19 looks for a period after non-zero numbers without checking the termination flag.
	if (needPeriod)
		ctx.WriteByte(basePtr + len, '.');
...
void ATAccelIFP(IATFPAccelContext& ctx) {
	const int value0 = ctx.ReadByte(ATKernelSymbols::FR0) + ((uint32)ctx.ReadByte(ATKernelSymbols::FR0+1) << 8);
	int value = value0;
	ATDecFloat r;
...
void ATAccelFPI(IATFPAccelContext& ctx) {
	const ATDecFloat x = ATReadFR0(ctx);
	...
	if (exp >= 0x43)
		...
		if (exp >= 0x3F) {
			roundbyte = x.mMantissa[exp - 0x3F];
	...
	if (value >= 0x10000) {
		...
		// The standard math pack has a bug: it rounds up _after_ checking
		// for overflow. Thus, 65535.5 gives 0 with no error.
		if (roundbyte >= 0x50)
			++value;

		ctx.WriteByte(0xD4, (uint8)value);
		ctx.WriteByte(0xD5, (uint8)(value >> 8));
		ctx.ClearFlagC();
...
void ATAccelFADD(IATFPAccelContext& ctx) {
	ATDecFloat fp0(ATReadFR0(ctx));
	ATDecFloat fp1(ATReadFR1(ctx));
	ATDecFloat fpr;
...
void ATAccelLOG(IATFPAccelContext& ctx) {
	const ATDecFloat x = ATReadFR0(ctx);
	const ATDecFloatOpt y = ATDecFloatOpt(x).Log();
	...
	if (g_ATLCFPAccel.IsEnabled())
		g_ATLCFPAccel("LOG(%s) -> %s\n", x.ToString().c_str(), y.Value().ToString().c_str());

	ATWriteFR0(ctx, y.Value());
	ctx.ClearFlagC();
}

void ATAccelLOG10(IATFPAccelContext& ctx) {
	const ATDecFloat x = ATReadFR0(ctx);
	const ATDecFloatOpt y = ATDecFloatOpt(x).Log10();
...
void ATAccelEXP(IATFPAccelContext& ctx) {
	const ATDecFloat x = ATReadFR0(ctx);
	const ATDecFloatOpt y = ATDecFloatOpt(x).Exp();
...
void ATAccelEXP10(IATFPAccelContext& ctx) {
	const ATDecFloat x = ATReadFR0(ctx);
	const ATDecFloatOpt y = ATDecFloatOpt(x).Exp10();
