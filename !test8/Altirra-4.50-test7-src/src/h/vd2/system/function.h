...
public:
	...
	vdfunction(vdfunction& src) = default; // : vdfuncbase(src) {}	// needed to avoid invoking (F&&)
	vdfunction(const vdfunction& src) = default; // : vdfuncbase(src) {}
	template<class F> vdfunction(std::reference_wrapper<F> f);

#ifdef VD_COMPILER_CLANG
	// Clang 19 and 22 have a bug with not being able to match member templates with
	// constraints, so we must use SFINAE instead.
	template<class F, typename = std::enable_if<!std::is_same_v<vdfunction<R(Args...)>, std::remove_cv_t<F>> && requires(F f) { f(std::declval<Args>()...); }>::type>
#else
	template<class F> requires(!std::is_same_v<vdfunction<R(Args...)>, std::remove_cv_t<F>> && requires(F f) { f(std::declval<Args>()...); })
#endif
	vdfunction(F&& f);
