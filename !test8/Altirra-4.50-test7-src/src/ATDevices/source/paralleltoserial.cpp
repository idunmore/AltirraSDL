...
public:		// IATDeviceParent
	IATDeviceBus *GetDeviceBusById(sint32 id) override;
	sint32 GetDeviceBusIdByIndex(uint32 index) override;
...
IATDeviceBus *ATDeviceParallelToSerial::GetDeviceBusById(sint32 id) {
	if (id == 0)
		return &mSerialPort;
	else
		return nullptr;
...
sint32 ATDeviceParallelToSerial::GetDeviceBusIdByIndex(uint32 index) {
	return index == 0 ? 0 : -1;
}
