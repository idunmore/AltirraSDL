...
void ATDiskFSSDX2::EnsureFreeSpace(uint32 minFreeSectors) {
	...
	IATEditableDiskImage *editableImage = vdpoly_cast<IATEditableDiskImage *>(mpImage);

	if (!mbAllowExtend || mpImage->IsDynamic() || !editableImage)
	...
	// Extend the disk.
	editableImage->Resize(requiredNewSize);
