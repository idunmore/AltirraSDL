...
namespace {
	...
	// 1,10,9,18,8,17,7,16,6,15,5,14,4,13,3,12,2,11 (815)
	static constexpr auto kTrackInterleaveDD_16_1 = SectorOrderToSectorPosition<
		1,10,9,18,8,17,7,16,6,15,5,14,4,13,3,12,2,11
	>();
...
ATDiskImagePSecIter::ATDiskImagePSecIter(const IATDiskImage& image, uint32 psec, uint32 count)
	: mpImage(&image)
	, mPhysIndex(psec)
	, mPhysCount(count)
{
...
void ATDiskImagePSecIter::operator++() {
	if (++mPhysOrder < mPhysCount)
		mPhysIndex = mpImage->GetNextPhysicalSector(mPhysIndex);
}

ATDiskImagePSecIterView ATDiskImagePSecIter::operator*() const {
	ATDiskImagePSecIterView view;

	view.mPhysIndex = mPhysIndex;
	view.mPhysOrder = mPhysOrder;

	mpImage->GetPhysicalSectorInfo(mPhysIndex, view.mPhysInfo);
...
ATDiskImagePSecRange::ATDiskImagePSecRange(const IATDiskImage& image, uint32 vsec)
	: mpImage(&image)
{
	ATDiskVirtualSectorInfo vsi;

	if (vsec >= image.GetVirtualSectorCount()) {
		mPhysStartIndex = 0;
		mPhysCount = 0;
	} else {
		image.GetVirtualSectorInfo(vsec, vsi);

		mPhysStartIndex = vsi.mFirstPhysSector;
		mPhysCount = vsi.mNumPhysSectors;
	}
}

////////////////////////////////////////////////////////////////////////////////

class ATDiskImage final : public vdrefcounted<IATEditableDiskImage> {
public:
	ATDiskImage();
	...
	bool IsValidPhysicalSector(uint32 index) const override;
	void GetPhysicalSectorInfo(uint32 index, ATDiskPhysicalSectorInfo& info) const override;
	uint32 GetNextPhysicalSector(uint32 physIndex) const override;
	...
	uint32 AddPhysicalSector(uint32 virtIndex, const ATDiskPhysicalSectorInfo& info, bool allocImageSpace, const void *optData, size_t dataLen) override;
	void RemovePhysicalSector(uint32 physIndex) override;
	void RemoveAllPhysicalSectorsForVirtualSector(uint32 virtIndex) override;

	void UpdatePhysicalSector(uint32 physIndex, const ATDiskPhysicalSectorInfo& info) override;
	...
	void AddDiskEditHandler(const DiskEditHandler *handler) override;
	void RemoveDiskEditHandler(const DiskEditHandler *handler) override;

	bool IsSafeToReinterleave() const override;
	void Reinterleave(ATDiskInterleave interleave) override;

protected:
	struct VirtSectorInfo : public ATDiskVirtualSectorInfo {
	};

	struct PhysSectorInfo : public ATDiskPhysicalSectorInfo {
		uint32 mNextPhysIndex = UINT32_MAX;
		uint32 mPrevPhysIndex = UINT32_MAX;
		uint32 mVirtIndex = UINT32_MAX;
	};

	typedef vdvector<PhysSectorInfo> PhysSectors;
	typedef vdvector<VirtSectorInfo> VirtSectors;
	...
	void ClearSectorsAndFreeList();
	void NotifyDiskEdit(ATDiskEditEvent event, uint32 virtIndex, uint32 physIndex);
	...
	vdfastvector<uint32> mFreePhysSectors;

	ATDiskImageFreeList mImageFreeList;
	ATNotifyList<const DiskEditHandler *> mDiskEditHandlers;
};
...
void ATDiskImage::Init(uint32 sectorCount, uint32 bootSectorCount, uint32 sectorSize) {
	...
	for(uint32 i=0; i<sectorCount; ++i) {
		psi.mImageOffset= i < mBootSectorCount ? 128*i : 128*mBootSectorCount + mSectorSize*(i-mBootSectorCount);
		psi.mDiskOffset	= -1;
		...
		AddPhysicalSector(i, psi, false, nullptr, 0);
	}
...
void ATDiskImage::LoadXFD(IVDRandomAccessStream& stream, sint64 fileSize) {
	...
	ATDiskPhysicalSectorInfo psi;
	for(uint32 i=0; i<sectorCount; ++i) {
		psi.mImageOffset= mSectorSize*i;
		psi.mDiskOffset	= psi.mImageOffset;
...
void ATDiskImage::LoadDCM(IVDRandomAccessStream& stream, uint32 len, const wchar_t *origPath, const uint8 *header) {
	...
	for(;;) {
		...
		for(;;) {
			...
			if (mVirtSectors.size() < sectorNum)
				mVirtSectors.resize(sectorNum);

			ATDiskPhysicalSectorInfo psi;
			psi.mImageOffset = (uint32)mImage.size();
			psi.mDiskOffset = -1;
			...
			AddPhysicalSector(sectorNum - 1, psi, false, nullptr, 0);
	...
	if (mVirtSectors.size() < mainSectorCount)
		mVirtSectors.resize(mainSectorCount);
	...
	for(VirtSectors::iterator it(mVirtSectors.begin()), itEnd(mVirtSectors.end()); it != itEnd; ++it) {
		...
		if (!vsi.mNumPhysSectors) {
			...
			AddPhysicalSector(secNum - 1, psi, false, nullptr, 0);
...
void ATDiskImage::LoadATX(IVDRandomAccessStream& stream, uint32 len, const uint8 *header) {
	...
	for(;;) {
		...
		for(uint32 k=0; k<trkhdr.mNumSectors; ++k) {
			...
			ATDiskPhysicalSectorInfo& psi = psec.mInfo;
			psi.mFDCStatus = ~sechdr.mFDCStatus | 0xc0;		// purpose of bit 7 is unknown
			psi.mImageOffset = (sint32)mImage.size();
		...
		for(uint32 chunkOffset = trkhdr.mDataOffset;;) {
			...
			if (ch.mType == ATXTrackChunkHeader::kTypeWeakBits || 
				...
				ATDiskPhysicalSectorInfo& psi = psecs[phyIndex].mInfo;
	...
	if (isTrack0MFM) {
		...
		for(const PhysicalSectorId& psecId : psecIds) {
			...
			for(sint32 psIndex = psecId.mFirstPSec; psIndex >= 0; psIndex = psecs[psIndex].mNext) {
				const ATDiskPhysicalSectorInfo& psi = psecs[psIndex].mInfo;
	...
	for(const PhysicalSectorId& psecId : psecIds) {
		...
		for(sint32 psecIndex = psecId.mFirstPSec; psecIndex >= 0; ) {
			...
			AddPhysicalSector(vsecIndex, psec.mInfo, false, nullptr, 0);
	...
	for(uint32 vsecIndex = 0; vsecIndex < numVsecs; ++vsecIndex) {
		...
		uint32 psecIndex = vsi.mFirstPhysSector;
		for(uint32 i = 0; i < vsi.mNumPhysSectors; ++i) {
			PhysSectorInfo& psi = mPhysSectors[psecIndex];
...
void ATDiskImage::LoadP2(IVDRandomAccessStream& stream, uint32 len, const uint8 *header) {
	...
	for(int i=0; i<sectorCount; ++i) {
		...
		psi.mImageOffset= (128+12)*i+12;
		psi.mDiskOffset = psi.mImageOffset + 16;
		psi.mImageSize		= 128;
		...
		if (phantomSectorCount) {
			...
			for(uint32 j=0; j<phantomSectorCount; ++j) {
				...
				ATDiskPhysicalSectorInfo psi;
				psi.mImageOffset= phantomSectorByteOffset+12;
				psi.mDiskOffset = psi.mImageOffset + 16;
				psi.mImageSize		= 128;
				psi.mPhysicalSize	= 128;
...
void ATDiskImage::LoadATR(IVDRandomAccessStream& stream, uint32 len, const wchar_t *origPath, const uint8 *header) {
	...
	for(uint32 i=0; i<sectorCount; ++i) {
		...
		if (packedBootSectors && i < 3) {
			psi.mImageOffset = 128*i;
		} else {
			psi.mImageOffset = i < imageBootSectorCount ? 128*i : 128*imageBootSectorCount + mSectorSize*(i-imageBootSectorCount);
		}
...
void *ATDiskImage::AsInterface(uint32 id) {
	switch(id) {
		...
		case IATEditableDiskImage::kTypeID: return static_cast<IATEditableDiskImage *>(this);
	}
...
void ATDiskImage::Flush() {
	...
	for(PhysSectorInfo* psi : dirtySectors) {
		...
		f.Write(&mImage[psi->mImageOffset], psi->mImageSize);
		psi->mbDirty = false;
	}
...
bool ATDiskImage::IsValidPhysicalSector(uint32 index) const {
	if (index >= mPhysSectors.size())
		return false;

	const auto& ps = mPhysSectors[index];

	if (ps.mVirtIndex == UINT32_MAX)
		return false;
...
uint32 ATDiskImage::GetNextPhysicalSector(uint32 physIndex) const {
	return mPhysSectors[physIndex].mNextPhysIndex;
}
...
void ATDiskImage::ReadPhysicalSector(uint32 index, void *data, uint32 len) {
	...
	memcpy(data, mImage.data() + psec.mImageOffset, copyLen);
...
void ATDiskImage::WritePhysicalSector(uint32 index, const void *data, uint32 len, uint8 fdcStatus) {
	...
	if (psi.mImageSize < len) {
		mbDiskFormatDirty = true;

		mImageFreeList.Free(psi.mImageOffset, psi.mImageSize, mImage);
		psi.mImageSize = psi.mPhysicalSize;
		psi.mImageOffset = mImageFreeList.Allocate(psi.mImageSize, mImage);
	}

	uint8 *dst = mImage.data() + psi.mImageOffset;
	memcpy(dst, data, len);

	if (len < psi.mImageSize)
		memset(dst + len, 0, psi.mImageSize - len);
...
uint32 ATDiskImage::ReadVirtualSector(uint32 index, void *data, uint32 len) {
	...
	const PhysSectorInfo& psi = mPhysSectors[vsi.mFirstPhysSector];
	...
	if (len) {
		...
		memcpy(data, mImage.data() + psi.mImageOffset, copyLen);
...
bool ATDiskImage::WriteVirtualSector(uint32 index, const void *data, uint32 len) {
	...
	PhysSectorInfo& psi = mPhysSectors[vsi.mFirstPhysSector];

	if (len < psi.mImageSize) {
		memcpy(mImage.data() + psi.mImageOffset, data, len);
		memset(mImage.data() + psi.mImageOffset + len, 0, psi.mImageSize - len);
	} else {
		memcpy(mImage.data() + psi.mImageOffset, data, std::min<uint32>(len, psi.mImageSize));
	}
	psi.mbDirty = true;
	mbDirty = true;
	mImageFileCRC.reset();
...
uint32 ATDiskImage::AddPhysicalSector(uint32 virtIndex, const ATDiskPhysicalSectorInfo& info, bool allocImageSpace, const void *optData, size_t dataLen) {
	if (virtIndex >= mVirtSectors.size()) {
		VDFAIL("Invalid virtual sector index.");
		VDRaiseInternalFailure();
	}

	auto& vsi = mVirtSectors[virtIndex];

	uint32 physIndex;
	
	if (mFreePhysSectors.empty()) {
		physIndex = (uint32)mPhysSectors.size();

		mPhysSectors.emplace_back();
	} else {
		physIndex = mFreePhysSectors.back();
		mFreePhysSectors.pop_back();
	}

	auto& psi = mPhysSectors[physIndex];

	static_cast<ATDiskPhysicalSectorInfo&>(psi) = info;

	psi.mVirtIndex = virtIndex;

	if (vsi.mNumPhysSectors == 0) {
		psi.mPrevPhysIndex = physIndex;
		psi.mNextPhysIndex = physIndex;
		vsi.mFirstPhysSector = physIndex;
	} else {
		const uint32 lastPhysIndex = mPhysSectors[vsi.mFirstPhysSector].mPrevPhysIndex;

		mPhysSectors[lastPhysIndex].mNextPhysIndex = physIndex;
		psi.mPrevPhysIndex = lastPhysIndex;
		psi.mNextPhysIndex = vsi.mFirstPhysSector;
		mPhysSectors[vsi.mFirstPhysSector].mPrevPhysIndex = physIndex;
	}
	...
	if (allocImageSpace) {
		...
		if (psi.mImageSize > 0) {
			void *sectorBuf = &mImage[psi.mImageOffset];
			memset(sectorBuf, 0, psi.mImageSize);

			if (optData && dataLen > 0) {
				memcpy(sectorBuf, optData, std::min<size_t>(psi.mImageSize, dataLen));
			}
		}
	}
	...
	NotifyDiskEdit(ATDiskEditEvent::PhysSectorAdded, virtIndex, physIndex);

	return physIndex;
}

void ATDiskImage::RemovePhysicalSector(uint32 physIndex) {
	if (physIndex >= mPhysSectors.size()) {
		VDFAIL("Invalid physical sector index.");
		return;
	}

	auto& psi = mPhysSectors[physIndex];

	if (psi.mVirtIndex == UINT32_MAX) {
		VDFAIL("Attempt to free already freed physical sector.");
		return;
	}

	auto& vsi = mVirtSectors[psi.mVirtIndex];
	if (vsi.mNumPhysSectors == 0) {
		VDFAIL("Corrupted physical sector chain detected.");
		return;
	}

	NotifyDiskEdit(ATDiskEditEvent::PhysSectorRemoving, psi.mVirtIndex, physIndex);

	mFreePhysSectors.push_back(physIndex);

	if (!--vsi.mNumPhysSectors) {
		vsi.mFirstPhysSector = 0;
	} else {
		PhysSectorInfo& psiPrev = mPhysSectors[psi.mPrevPhysIndex];
		PhysSectorInfo& psiNext = mPhysSectors[psi.mNextPhysIndex];

		psiPrev.mNextPhysIndex = psi.mNextPhysIndex;
		psiNext.mPrevPhysIndex = psi.mPrevPhysIndex;

		if (vsi.mFirstPhysSector == physIndex)
			vsi.mFirstPhysSector = psi.mNextPhysIndex;
	}

	psi.mPrevPhysIndex = UINT32_MAX;
	psi.mNextPhysIndex = UINT32_MAX;
	psi.mVirtIndex = UINT32_MAX;
...
void ATDiskImage::RemoveAllPhysicalSectorsForVirtualSector(uint32 virtIndex) {
	if (virtIndex >= mVirtSectors.size()) {
		VDFAIL("Invalid virtual sector index.");
		return;
	}
	...
	while(vsi.mNumPhysSectors)
		RemovePhysicalSector(vsi.mFirstPhysSector);
}

void ATDiskImage::UpdatePhysicalSector(uint32 physIndex, const ATDiskPhysicalSectorInfo& info) {
	if (physIndex >= mPhysSectors.size()) {
		VDFAIL("Invalid physical sector index.");
		return;
	}
	...
	// Check if the physical size has changed.
	if (psi.mPhysicalSize != info.mPhysicalSize) {
		psi.mPhysicalSize = info.mPhysicalSize;

		// mark the sector and disk format as dirty
		psi.mbDirty = true;
		mbDirty = true;
		mbDiskFormatDirty = true;
	}
	...
	if (psi.mImageSize != info.mImageSize) {
		// check if we are shrinking or expanding the sector relative to its
		// _image_ size
		if (psi.mImageSize > info.mImageSize) {
			// shrinking -- just free the extra image space if it exists
			mImageFreeList.Free(psi.mImageOffset + info.mImageSize, psi.mImageSize - info.mImageSize, mImage);

			// clear the image offset if there is no sector data (possible with
			// address CRC errors)
			if (!info.mImageSize)
				psi.mImageOffset = 0;
		} else if (psi.mImageSize < info.mImageSize) {
			// expanding -- copy out the sector data
			vdfastvector<uint8> sectorData;

			if (psi.mImageSize)
				sectorData.assign(mImage.begin() + psi.mImageOffset, mImage.begin() + psi.mImageOffset + psi.mImageSize);

			// free the old sector image space
			mImageFreeList.Free(psi.mImageOffset, psi.mImageSize, mImage);

			// allocate new sector image space
			psi.mImageOffset = mImageFreeList.Allocate(info.mImageSize, mImage);

			// clear new image space
			memset(&mImage[psi.mImageOffset], 0, info.mImageSize);

			// copy back existing image data
			if (!sectorData.empty())
				memcpy(&mImage[psi.mImageOffset], sectorData.data(), sectorData.size());
		}

		psi.mImageSize = info.mImageSize;

		// invalidate the disk offset
		psi.mDiskOffset = -1;
	...
	psi.mbMFM = info.mbMFM;
	psi.mRotPos = info.mRotPos - floorf(info.mRotPos);
	psi.mFDCStatus = info.mFDCStatus;
	psi.mWeakDataOffset = info.mWeakDataOffset < 0 ? -1 : info.mWeakDataOffset;
...
void ATDiskImage::Resize(uint32 newSectors) {
	...
	if (newSectors < curSectors) {
		...
		for(uint32 vsecCount = curSectors; vsecCount > newSectors; --vsecCount) {
			VirtSectorInfo& vsi = mVirtSectors.back();

			while(vsi.mNumPhysSectors)
				RemovePhysicalSector(vsi.mFirstPhysSector);

			mVirtSectors.pop_back();
		}
		...
		vdvector<Span> imageSpans;

		// Extract spans from all remaining physical sectors.
		for(auto& ps : mPhysSectors) {
			if (ps.mImageSize)
				imageSpans.emplace_back(Span { ps.mImageOffset, ps.mImageOffset + ps.mImageSize });
		}
		...
		for(auto& ps : mPhysSectors) {
			ps.mImageOffset = std::lower_bound(imageSpans.begin(), imageSpans.end(), ps.mImageOffset,
		...
		// Add new virtual sectors.
		mVirtSectors.resize(newSectors);

		// Add new physical sectors.
		for(uint32 i = 0; i < sectorsToAdd; ++i) {
			ATDiskPhysicalSectorInfo ps;
			ps.mbDirty = true;
			ps.mDiskOffset = -1;
			ps.mFDCStatus = 0xFF;
			ps.mImageOffset = imageOffset + mSectorSize * i;
			ps.mRotPos = 0;
			ps.mImageSize = mSectorSize;
			ps.mPhysicalSize = mSectorSize;
			ps.mWeakDataOffset = -1;

			AddPhysicalSector(curSectors + i, ps, false, nullptr, 0);
		}
...
void ATDiskImage::FormatTrack(uint32 vsIndexStart, uint32 vsCount, const ATDiskVirtualSectorInfo *vsecs, uint32 psCount, const ATDiskPhysicalSectorInfo *psecs, const uint8 *psecData) {
	...
	if (mVirtSectors.size() < vsIndexStart + vsCount)
		mVirtSectors.resize(vsIndexStart + vsCount);

	// Delete all of the original sectors and add in the new physical sectors.
	for(uint32 i = 0; i < vsCount; ++i) {
		RemoveAllPhysicalSectorsForVirtualSector(vsIndexStart + i);

		const auto& srcVSec = vsecs[i];
		for(uint32 j = 0; j < srcVSec.mNumPhysSectors; ++j) {
			const auto& srcPSec = psecs[srcVSec.mFirstPhysSector + j];

			AddPhysicalSector(vsIndexStart + i, srcPSec, true, srcPSec.mImageSize ? psecData + srcPSec.mImageOffset : nullptr, srcPSec.mImageSize);
		}
	}
	...
	if (vsIndexStart == 0 && vsCount > 0) {
		const ATDiskVirtualSectorInfo& vsi0 = mVirtSectors[0];

		if (vsi0.mNumPhysSectors) {
			const ATDiskPhysicalSectorInfo *psis = &mPhysSectors[vsi0.mFirstPhysSector];
...
void ATDiskImage::AddDiskEditHandler(const vdfunction<void(ATDiskEditEvent, uint32 /*vsec*/, uint32 /*psec*/)>* handler) {
	mDiskEditHandlers.Add(handler);
}

void ATDiskImage::RemoveDiskEditHandler(const vdfunction<void(ATDiskEditEvent, uint32 /*vsec*/, uint32 /*psec*/)>* handler) {
	mDiskEditHandlers.Remove(handler);
}
...
void ATDiskImage::Reinterleave(ATDiskInterleave interleave) {
	...
	for(const ATDiskVirtualSectorInfo& vsi : mVirtSectors) {
		...
		if (n) {
			uint32 psec = vsi.mFirstPhysSector;
			float pos = interleaveFn(vsIndex);
			float posInc = 1.0f / (float)n;

			for(uint32 i=0; i<n; ++i) {
				mPhysSectors[psec].mRotPos = pos;
				pos += posInc;

				psec = mPhysSectors[psec].mNextPhysIndex;
			}
		}
...
void ATDiskImage::SaveATR(IVDRandomAccessStream& fs, PhysSectors& phySecs) {
	...
	for(uint32 vsIndex = 0; vsIndex < numVsecs; ++vsIndex) {
		...
		if (vsi.mNumPhysSectors == 0) {
			sectorBuffer.clear();
			sectorBuffer.resize(writeSize, 0);

			f.Write(sectorBuffer.data(), writeSize);
			diskOffset += writeSize;
		} else {
			PhysSectorInfo& psi = phySecs[vsi.mFirstPhysSector];

			if (psi.mImageSize >= writeSize) {
				f.Write(&mImage[psi.mImageOffset], writeSize);
			} else {
				sectorBuffer.resize(writeSize);

				const uint32 copyLen = psi.mImageSize;
				if (copyLen)
					memcpy(sectorBuffer.data(), &mImage[psi.mImageOffset], copyLen);
...
void ATDiskImage::SaveXFD(IVDRandomAccessStream& fs, PhysSectors& phySecs) {
	...
	vdblock<char> sectorBuffer(mSectorSize);

	for(const VirtSectorInfo& vsi : mVirtSectors) {
		if (vsi.mNumPhysSectors == 0) {
			memset(sectorBuffer.data(), 0, mSectorSize);
			...
			if (psi.mImageSize < mSectorSize) {
				...
				f.Write(&mImage[psi.mImageOffset], mSectorSize);
...
void ATDiskImage::SaveP2(IVDRandomAccessStream& f, PhysSectors& phySecs) {
	...
	for(uint32 i=0; i<numSectors; ++i) {
		...
		uint32 physIndex = vsi.mFirstPhysSector;
		PhysSectorInfo *psi = &phySecs[physIndex];
		...
		for(uint32 j=1; j<vsi.mNumPhysSectors; ++j) {
			physIndex = psi->mNextPhysIndex;

			psi = &phySecs[physIndex];
	...
	for(uint32 i=0; i<totalSectors; ++i) {
		...
		if (psi) {
			memcpy(sectorData.mData, &mImage[psi->mImageOffset], 128);
...
void ATDiskImage::SaveP3(IVDRandomAccessStream& f, PhysSectors& phySecs) {
	...
	for(uint32 i=0; i<numSectors; ++i) {
		...
		for(uint32 j=1; j<vsi.mNumPhysSectors; ++j) {
			psi = &phySecs[psi->mNextPhysIndex];
...
void ATDiskImage::SaveDCM(IVDRandomAccessStream& f, PhysSectors& phySecs) {
	...
	for(uint32 sector=1; sector<=numSecs; ++sector) {
		...
		// if there is no physical sector, consider it all zeroes and skip it
		if (!vsi.mNumPhysSectors)
			continue;
		...
		// to 256b in DD mode
		const uint32 copyLen = std::min<uint32>(mSectorSize, psi.mImageSize);
		memcpy(nextBuf, &mImage[psi.mImageOffset], copyLen);
...
void ATDiskImage::SaveATX(IVDRandomAccessStream& fs, PhysSectors& phySecs) {
	...
	for(uint32 i = 0; i < totalVsecs; ++i) {
		...
		for(uint32 j = 0; j < vsi.mNumPhysSectors; ++j) {
			const PhysSectorInfo& psi = phySecs[physIndex];
...
void ATDiskImage::ClearSectorsAndFreeList() {
	mVirtSectors.clear();
	mPhysSectors.clear();
	mFreePhysSectors.clear();
	mImageFreeList.Clear();
}

void ATDiskImage::NotifyDiskEdit(ATDiskEditEvent event, uint32 virtIndex, uint32 physIndex) {
	mDiskEditHandlers.NotifyAllDirect(
		[=](const DiskEditHandler *handler) {
			(*handler)(event, virtIndex, physIndex);
		}
	);
}
...
vdfunction<float(uint32)> ATDiskGetInterleaveFn(ATDiskInterleave interleave, const ATDiskGeometryInfo& info) {
	...
	switch(interleave) {
		...
		case kATDiskInterleave_DD_16_1:
			return [](uint32 secIdx) { return (float)kTrackInterleaveDD_16_1[secIdx % 18] * kTurnsPerSectorDD; };
