...
bool ATUINativeWindowProxy::HasFocus() const {
	return mhwnd && ::GetFocus() == mhwnd;
}

bool ATUINativeWindowProxy::HasNestedFocus() const {
	if (!mhwnd)
		return false;

	HWND hwndFocus = ::GetFocus();

	for(HWND hwndTest = hwndFocus; hwndTest; hwndTest = ::GetAncestor(hwndTest, GA_PARENT)) {
		if (hwndTest == mhwnd)
			return true;
	}
...
void ATUINativeWindowProxy::SetClipSiblings(bool enabled) {
	if (mhwnd) {
		DWORD flags = GetWindowLong(mhwnd, GWL_STYLE);
		DWORD newFlags = flags;

		if (enabled)
			newFlags |= WS_CLIPSIBLINGS;
		else
			newFlags &= ~WS_CLIPSIBLINGS;

		if (flags != newFlags)
			SetWindowLong(mhwnd, GWL_STYLE, newFlags);
	}
}
...
void ATUINativeWindowProxy::BringToFrontTopmost() {
	if (!mhwnd)
		return;
...
void ATUINativeWindowProxy::SendToBack() {
	if (!mhwnd)
		return;
...
void ATUINativeWindowProxy::SetCapture() {
	if (mhwnd)
		::SetCapture(mhwnd);
}

void ATUINativeWindowProxy::ReleaseCapture() {
	if (mhwnd && ::GetCapture() == mhwnd)
		::ReleaseCapture();
}

VDZHMENU ATUINativeWindowProxy::GetAttachedMenu() const {
	if (!mhwnd)
		return nullptr;
