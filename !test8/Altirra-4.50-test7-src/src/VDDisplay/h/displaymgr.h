...
public:
	...
	bool	IsMonitorWCGCapable(HMONITOR monitor);
	bool	IsMonitorWCGEnabled(HMONITOR monitor);
	void	GetMonitorInformation(HMONITOR monitor, VDDMonitorInfo& info);

private:
	struct MonitorHDRInfo;

	const MonitorHDRInfo& GetMonitorHDRInfo(HMONITOR monitor);
	void	UpdateMonitorInfoCache();
	void	UpdateDisplayConfigInfoCache();
	void	UpdateDXGIInfoCache();
	...
	ATOM		mWndClass;
	HWND		mhwnd;
	vdrefptr<VDDXGIHolder> mpDXGIHolder;
	...
	struct MonitorHDRInfo : public MonitorHDRInfoFromDC {
		...
		float mMaxLuminance = 0.0f;
		float mMaxFullFrameLuminance = 0.0f;
	};
	...
	vdvector<MonitorHDRInfoByHandle> mMonitorHDRInfoCache;

	struct MonitorHDRInfoByPath {
		uint32 mPathHash = 0;
		MonitorHDRInfoFromDC mInfo;
	};

	vdvector<MonitorHDRInfoByPath> mMonitorHDRInfoCacheDC;
	bool mbMonitorHDRInfoCacheDCValid = false;

	vdrefptr<IDXGIFactory6> mpDXGIFactory6;
	bool mbDXGI16Available = true;
