...
bool VDVideoDisplayManager::Init() {
	...
	mpDXGIHolder = new VDDXGIHolder;
	if (!mpDXGIHolder->Init())
		mpDXGIHolder = nullptr;
...
bool VDVideoDisplayManager::IsMonitorWCGCapable(HMONITOR monitor) {
	return GetMonitorHDRInfo(monitor).mbWCGCapable;
}

bool VDVideoDisplayManager::IsMonitorWCGEnabled(HMONITOR monitor) {
	return GetMonitorHDRInfo(monitor).mbWCGEnabled;
}

void VDVideoDisplayManager::GetMonitorInformation(HMONITOR monitor, VDDMonitorInfo& info) {
	const auto& src = GetMonitorHDRInfo(monitor);

	info.mbValid = true;
	info.mDisplayName = src.mDisplayName;
	info.mbHDRCapable = src.mbHDRCapable;
	info.mbHDREnabled = src.mbHDREnabled;
	info.mbWCGCapable = src.mbWCGCapable;
	info.mbWCGEnabled = src.mbWCGEnabled;

	info.mRedPrimary = src.mRedPrimary;
	info.mGreenPrimary = src.mGreenPrimary;
	info.mBluePrimary = src.mBluePrimary;
	info.mWhitePoint = src.mWhitePoint;
	info.mMaxLuminance = src.mMaxLuminance;
	info.mMaxFullFrameLuminance = src.mMaxFullFrameLuminance;
}

const VDVideoDisplayManager::MonitorHDRInfo& VDVideoDisplayManager::GetMonitorHDRInfo(HMONITOR monitor) {
	const auto pred = [=](const MonitorHDRInfoByHandle& info) { return info.mhMonitor == monitor; };
	auto it = std::find_if(mMonitorHDRInfoCache.begin(), mMonitorHDRInfoCache.end(), pred);

	if (it != mMonitorHDRInfoCache.end())
		return it->mInfo;

	UpdateMonitorInfoCache();

	it = std::find_if(mMonitorHDRInfoCache.begin(), mMonitorHDRInfoCache.end(), pred);
	
	if (it == mMonitorHDRInfoCache.end()) {
		mMonitorHDRInfoCache.emplace_back();
		it = mMonitorHDRInfoCache.end() - 1;
	}
	...
	if (GetMonitorInfoW(monitor, &info)) {
		...
		for(const MonitorHDRInfoByPath& mipath : mMonitorHDRInfoCacheDC) {
			if (mipath.mPathHash == pathHash) {
				static_cast<MonitorHDRInfoFromDC&>(monent.mInfo) = mipath.mInfo;
				break;
			}
		}
	}
...
void VDVideoDisplayManager::UpdateMonitorInfoCache() {
	...
	UpdateDisplayConfigInfoCache();
	UpdateDXGIInfoCache();
}

void VDVideoDisplayManager::UpdateDisplayConfigInfoCache() {
	...
	for(const DISPLAYCONFIG_PATH_INFO& path : paths) {
		...
		DISPLAYCONFIG_TARGET_DEVICE_NAME devtargetname {};
		devtargetname.header.adapterId = path.targetInfo.adapterId;
		devtargetname.header.id = path.targetInfo.id;
		devtargetname.header.type = DISPLAYCONFIG_DEVICE_INFO_GET_TARGET_NAME;
		devtargetname.header.size = sizeof devtargetname;

		if (ERROR_SUCCESS == DisplayConfigGetDeviceInfo(&devtargetname.header)) {
			if (devtargetname.monitorFriendlyDeviceName[0] == 0 && devtargetname.outputTechnology == DISPLAYCONFIG_OUTPUT_TECHNOLOGY_INTERNAL)
				ent.mInfo.mDisplayName = L"Internal display";
			else
				ent.mInfo.mDisplayName = devtargetname.monitorFriendlyDeviceName;
		}
		...
		if (ERROR_SUCCESS == DisplayConfigGetDeviceInfo(&getinfo2.header)) {
			...
			ent.mInfo.mbWCGCapable = ent.mInfo.mbHDRCapable || getinfo2.wideColorSupported;
			ent.mInfo.mbWCGEnabled = ent.mInfo.mbHDREnabled || getinfo2.activeColorMode == DISPLAYCONFIG_ADVANCED_COLOR_MODE_WCG;
		} else {
			...
			if (ERROR_SUCCESS == DisplayConfigGetDeviceInfo(&getinfo.header)) {
				...
				ent.mInfo.mbWCGCapable = ent.mInfo.mbHDRCapable;
				ent.mInfo.mbWCGEnabled = ent.mInfo.mbHDREnabled;
			}
		}
	}
}

void VDVideoDisplayManager::UpdateDXGIInfoCache() {
	if (!mbDXGI16Available)
		return;

	if (mpDXGIFactory6 && !mpDXGIFactory6->IsCurrent())
		mpDXGIFactory6 = nullptr;

	if (!mpDXGIFactory6) {
		if (!mpDXGIHolder) {
			mbDXGI16Available = false;
			return;
		}

		auto create2 = mpDXGIHolder->GetCreateDXGIFactory2Fn();
		if (!create2) {
			mbDXGI16Available = false;
			return;
		}

		HRESULT hr = create2(0, IID_IDXGIFactory6, (void **)~mpDXGIFactory6);
		if (FAILED(hr)) {
			mbDXGI16Available = false;
			return;
		}
	...
	for(UINT adapterIdx = 0; SUCCEEDED(mpDXGIFactory6->EnumAdapters1(adapterIdx, ~adapter)); ++adapterIdx) {
		...
		for(UINT outputIdx = 0; SUCCEEDED(adapter->EnumOutputs(outputIdx, ~output)); ++outputIdx) {
			...
			if (SUCCEEDED(output->QueryInterface(IID_IDXGIOutput6, (void **)~output6))) {
				...
				if (SUCCEEDED(output6->GetDesc1(&desc))) {
					auto it = std::find_if(mMonitorHDRInfoCache.begin(),
						mMonitorHDRInfoCache.end(),
						[mon = desc.Monitor](const MonitorHDRInfoByHandle& info) {
							return info.mhMonitor == mon;
						}
					);
					...
					if (it == mMonitorHDRInfoCache.end()) {
						...
						it = mMonitorHDRInfoCache.end() - 1;
						it->mhMonitor = desc.Monitor;
					}

					it->mInfo.mRedPrimary.x = desc.RedPrimary[0];
					it->mInfo.mRedPrimary.y = desc.RedPrimary[1];
					it->mInfo.mGreenPrimary.x = desc.GreenPrimary[0];
					it->mInfo.mGreenPrimary.y = desc.GreenPrimary[1];
					it->mInfo.mBluePrimary.x = desc.BluePrimary[0];
					it->mInfo.mBluePrimary.y = desc.BluePrimary[1];
					it->mInfo.mWhitePoint.x = desc.WhitePoint[0];
					it->mInfo.mWhitePoint.y = desc.WhitePoint[1];
					it->mInfo.mMaxLuminance = desc.MaxLuminance;
					it->mInfo.mMaxFullFrameLuminance = desc.MaxFullFrameLuminance;
				}
			}
		}
	}
